...
def buildImage(def buildContext, def imageName, def tag) {
    sh """
        podman build -t ${imageName}:${tag} ${buildContext} --network=host
    """
}
...
def tagImageCustom(def imageName, def currentTag, def newTag) {
    sh """
        podman tag ${imageName}:${currentTag} ${imageName}:${newTag}
    """
}

def tagImage(def imageName, def currentTag, def newTag) {
    sh """
        podman tag docker.io/${imageName}:${currentTag} docker.io/${imageName}:${newTag}
    """
}
